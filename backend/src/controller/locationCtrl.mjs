...
const adminCreateLocation = async (req, res, next) => {
  ...
  try {
    ...
    const newLocation = new Location({ name: capitalizedName, county: capitalizedCounty, town: capitalizedTown });
...
const adminModifyLocation = async (req, res, next) => {
  ...
  try {
    ...
    if (name) locationToUpdate.name = await capitalizeFirstLetter(name);
    if (county) locationToUpdate.county = await capitalizeFirstLetter(county);
    if (town) locationToUpdate.town = await capitalizeFirstLetter(town);

    await locationToUpdate.save();

    // Return a success response
    return res.status(200).json({ success: true, message: 'Location Updated Successfully', details: locationToUpdate });
  } catch (error) {
    next(error);
  }
};
...
const getAllLocations = async (req, res, next) => {
  try {
    ...
    // Return a success response
    return res.status(200).json({ success: true, message: 'Location Retrieved Successfully', details: allLocations });
  } catch (error) {
    next(error);
  }
};
...
const viewOneLocation = async (req, res, next) => {
  ...
  try {
    ...
    // Return a success response
    return res.status(200).json({ success: true, message: 'Location successfully retrieved', details: location });
  } catch (error) {
    next(error);
  }
};
