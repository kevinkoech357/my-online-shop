    ...
    for (const item of items) {
        if (!item.product || !item.quantity || !item.color) {
            ...
            response.errors.push(
                "Each cart item must have product, quantity, and color",
            );
        }
...
const createOrUpdateCart = async (req, res) => {
    ...
    try {
        ...
        const products = await Product.find({
            _id: { $in: productIds },
        }).select("_id price quantity color name slug status");
...
const getUserCart = async (req, res, next) => {
    ...
    try {
        // Find the cart using the user's ID
        const existingCart = await Cart.findOne({ user: _id }).lean();
