	...
	try {
		...
		for (const newItem of cart.items) {
			...
			if (!isValid) {
				return res
					.status(400)
					.json({ success: false, message: "Invalid Product ID parameter." });
			}
			...
			if (existingItem) {
				...
				if (newItem.quantity > product.quantity) {
					return res.status(400).json({
						success: false,
						message: "Quantity exceeds the defined Limit.",
					});
				}
		...
		if (existingCart) {
			...
			return res
				.status(201)
				.json({ success: true, message: "Cart Updated", cart: existingCart });
		} else {
			...
			return res
				.status(201)
				.json({ success: true, message: "New Cart Created", cart: newCart });
		}
	} catch (error) {
		next(error);
	}
...
const removeProductFromCart = async (req, res, next) => {
	...
	try {
		...
		existingCart.cartTotal = existingCart.items.reduce(
