...
const adminModifyProductCategory = async (req, res, next) => {
  ...
  try {
    ...
    return res.status(200).json({ success: true, message: 'Product Category Updated Successfully', details: productCategoryToUpdate });
  } catch (error) {
    next(error);
  }
};
...
const getAllProductCategories = async (req, res, next) => {
  try {
    ...
    if (!allProductCategories) {
      ...
      return res.status(200).json({ success: true, message: 'No Product Categories Found.', details: [] });
    }

    // Return a success response
    return res.status(200).json({ success: true, message: 'Product Category Retrieved Successfully', details: allProductCategories });
  } catch (error) {
    next(error);
  }
};
...
const viewOneProductCategory = async (req, res, next) => {
  ...
  try {
    ...
    // Return a success response
    return res.status(200).json({ success: true, message: 'Product Category successfully retrieved', details: productCategory });
  } catch (error) {
    next(error);
  }
};
