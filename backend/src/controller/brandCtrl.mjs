...
const adminModifyBrand = async (req, res, next) => {
  ...
  try {
    ...
    return res.status(200).json({ success: true, message: 'Brand Updated Successfully', details: brandToUpdate });
  } catch (error) {
    next(error);
  }
};
...
const getAllBrands = async (req, res, next) => {
  try {
    ...
    // Return a success response
    return res.status(200).json({ success: true, message: 'Brand Retrieved Successfully', details: allBrands });
  } catch (error) {
    next(error);
  }
};
...
const viewOneBrand = async (req, res, next) => {
  ...
  try {
    ...
    // Return a success response
    return res.status(200).json({ success: true, message: 'Brand successfully retrieved', details: brand });
  } catch (error) {
    next(error);
  }
};
